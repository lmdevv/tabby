...
import { closeTabsSafely, getLastFocusedWindowIdSafe, isDashboardTab } from "@/entrypoints/background/utils";
import { cleanDuplicateTabsInWorkspace, cleanNonResourceTabsInWorkspace, cleanResourceTabsInWorkspace, cleanUnusedTabsInWorkspace } from "@/entrypoints/background/operations/cleaning-operations";
...
import { groupTabsInWorkspace, sortTabsInWorkspace, ungroupTabsInWorkspace } from "@/entrypoints/background/operations/tab-operations";
import { activateWorkspace, createWorkspaceFromUrls } from "@/entrypoints/background/operations/workspace-operations";
import { createWorkspaceSnapshot, deleteSnapshot, restoreSnapshot } from "@/entrypoints/background/snapshots";
...
type MessageHandler<T extends RuntimeMessage = RuntimeMessage> = (message: T, ctx: { getActiveWorkspace: () => Workspace | undefined }) => Promise<HandlerResult>;
...
const handlers: Partial<Record<RuntimeMessage["type"], MessageHandler<any>>> = {
  ...
  async toggleGroupCollapse(message) {
    ...
    await browser.tabGroups.update(message.groupId, { collapsed: !group.collapsed });
    return { success: true };
  },

  async openWorkspace(message) {
    await activateWorkspace(message.workspaceId, { skipTabSwitching: message.skipTabSwitching === true });
    return { success: true };
  },

  async closeWorkspace() {
    const currentActiveWorkspace = await db.workspaces.where("active").equals(1).first();
    if (currentActiveWorkspace) {
      ...
      await db.workspaces.where("id").equals(currentActiveWorkspace.id).modify({ active: 0 });
  ...
  async cleanUnusedTabs(message) {
    await cleanUnusedTabsInWorkspace(message.workspaceId, message.daysThreshold);
    return { success: true };
  },
  ...
  async openResourcesAsGroup(message) {
    ...
    for (const url of urls) {
      const tab = await browser.tabs.create({ url, windowId: targetWindowId, active: false });
      if (tab.id != null) {
        createdTabIds.push(tab.id);
        if (!targetWindowId && tab.windowId != null) targetWindowId = tab.windowId;
      }
    }
    if (createdTabIds.length >= 2 && targetWindowId != null) {
      const groupId = await browser.tabs.group({ tabIds: createdTabIds as [number, ...number[]], createProperties: { windowId: targetWindowId } });
      await browser.tabGroups.update(groupId, { title: message.title, color: getRandomTabGroupColor() });
    }
    return { success: true };
  },
  ...
  async createWorkspaceFromTabGroup(message) {
    ...
    const tabsInGroup = await db.activeTabs.where("groupId").equals(message.groupId).toArray();
    const urls = tabsInGroup.map((t) => t.url).filter((u): u is string => Boolean(u));
    ...
    // Close original tabs and archive group + tabs
    const tabIdsToClose = tabsInGroup.map((t) => t.id).filter((id): id is number => id != null);
    ...
    await db.transaction("rw", db.activeTabs, db.tabGroups, async () => {
      if (stableIdsToArchive.length > 0) {
        await db.activeTabs.where("stableId").anyOf(stableIdsToArchive).modify({ tabStatus: "archived" });
      }
      await db.tabGroups.update(message.groupId, { groupStatus: "archived", updatedAt: Date.now() });
    });
...
export function registerMessageHandlers(getActiveWorkspace: () => Workspace | undefined) {
  browser.runtime.onMessage.addListener(async (message: RuntimeMessage) => {
    try {
      if (typeof message !== "object" || !message) return;
      const handler = handlers[message.type];
      if (!handler) return;
      const res = await handler(message as any, { getActiveWorkspace });
      return res as HandlerResult;
    } catch (error) {
      console.error("Message handler error:", error);
      return { success: false, error: error instanceof Error ? error.message : String(error) } satisfies HandlerResult;
    }
  });
}
